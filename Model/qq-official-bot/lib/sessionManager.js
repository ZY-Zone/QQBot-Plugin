...
class SessionManager extends events_1.EventEmitter {
    constructor(bot) {
        ...
        this.on(constans_1.SessionEvents.EVENT_WS, (data) => {
            switch (data.eventType) {
                case constans_1.SessionEvents.RECONNECT:
                    this.bot.logger.mark("[TSBOT-CLIENT] 等待断线重连中...");
                    break;
                case constans_1.SessionEvents.DISCONNECT:
                    ...
                    if (this.retry < (this.bot.config.maxRetry || exports.MAX_RETRY)) {
                        this.bot.logger.mark("[TSBOT-CLIENT] 重新连接中，尝试次数：", this.retry + 1);
                        ...
                        this.bot.logger.mark("[TSBOT-CLIENT] 超过重试次数，连接终止");
                ...
                case constans_1.SessionEvents.READY:
                    this.bot.logger.mark("[TSBOT-CLIENT] 连接成功");
                    this.retry = 0;
                    break;
                default:
            }
        ...
        this.on(constans_1.SessionEvents.ERROR, (code, message) => {
            this.bot.logger.error(`[TSBOT-CLIENT] 发生错误：${code} ${message}`);
        });
    }
    ...
    async getWsUrl() {
      if (this.bot.config?.WsUrl?.startsWith('ws://') || this.bot.config?.WsUrl?.startsWith('wss://')){
        this.wsUrl = this.bot.config?.WsUrl;
        this.bot.logger.info(`[TSBOT-CLIENT] 使用自定义WsUrl`);
      ...
      let botInfo = this.BotInfo
        ...
        this.bot.ws.on("message", (data) => {
            ...
            if (wsRes.t === constans_1.SessionEvents.READY) {
                this.bot.logger.mark(`[TSBOT-CLIENT] 鉴权通过`);
                const { d, s } = wsRes;
                const { session_id } = d;
                let user = { id: botInfo?.id, username: botInfo?.username }
                ...
                // 第一次发送心跳
                this.bot.logger.debug(`[TSBOT-CLIENT] 发送第一次心跳`, this.heartbeatParam);
            ...
            if (wsRes.op === constans_1.OpCode.HEARTBEAT_ACK || wsRes.t === constans_1.SessionEvents.RESUMED) {
                ...
                this.bot.logger.debug("[TSBOT-CLIENT] 心跳校验", this.heartbeatParam);
                setTimeout(() => {
                    this.sendWs(this.heartbeatParam);
